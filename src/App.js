...
// --- Helper function to check for 2nd/4th Saturday ---
const isSecondOrFourthSaturday = (date) => {
    if (date.getDay() !== 6) return false; // Not a Saturday
    const dayOfMonth = date.getDate();
    return Math.ceil(dayOfMonth / 7) === 2 || Math.ceil(dayOfMonth / 7) === 4;
};
...
function App() {
        ...
        const unsub = onSnapshot(userDocRef, (doc) => {
          if (doc.exists()) {
            ...
            const initialData = {
              ...
              semesterEndDate: '',
              holidays: [],
              conductedClasses: 0, // NEW
              attendedClasses: 0,  // NEW
            };
  ...
  return (
    <div className={`flex min-h-screen bg-gray-100 dark:bg-gray-900 font-sans transition-colors duration-500`}>
      {user && <Sidebar setPage={setPage} currentPage={page} handleLogout={handleLogout} toggleTheme={toggleTheme} theme={theme} />}
...
// --- Sidebar Component ---
function Sidebar({ setPage, currentPage, handleLogout, toggleTheme, theme }) {
  ...
  return (
    <aside className="w-64 bg-white dark:bg-gray-800 shadow-xl flex flex-col transition-colors duration-500 p-4">
      <div className="text-2xl font-bold text-center py-6 text-blue-600 dark:text-blue-400">
        Attendify
      </div>
  ...
  return (
    <div className="flex flex-col items-center justify-center h-screen bg-gradient-to-br from-blue-500 to-indigo-600 dark:from-gray-800 dark:to-black">
        <div className="text-center p-8 bg-white/10 backdrop-blur-lg rounded-2xl shadow-2xl border border-white/20">
            <h1 className="text-5xl font-extrabold text-white mb-4 animate-fade-in-down">Welcome to Attendify</h1>
...
function Dashboard({ user, userData }) {
  ...
  const calculateAttendance = useCallback(() => {
    if (!userData || !userData.semesterStartDate || !userData.semesterEndDate || !userData.timetable.length) {
      return { currentPercent: 0, totalClasses: 0, attendedClasses: 0, futurePercent: 0, conductedClasses: 0 };
    }

    const { semesterStartDate, semesterEndDate, timetable, holidays, conductedClasses, attendedClasses } = userData;
    ...
    for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
        ...
        const isHoliday = allHolidays.includes(dateString) || dayOfWeek === 0 || isSecondOrFourthSaturday(new Date(d));

        if (!isHoliday) {
            const classesOnDay = timetable.filter(c => c.day === dayOfWeek.toString()).length;
            totalSemesterClasses += classesOnDay;
            if (d >= today) {
                futureClasses += classesOnDay;
            }
        }
    }
    ...
    // Prediction Logic
    let classesToSkip = 0;
    let tempFutureClasses = 0;
    let daysChecked = 0;
    for (let d = new Date(today); d <= endDate && daysChecked < predictionDays; d.setDate(d.getDate() + 1)) {
        const dayOfWeek = d.getDay();
        const dateString = d.toISOString().split('T')[0];
        const isHoliday = allHolidays.includes(dateString) || dayOfWeek === 0 || isSecondOrFourthSaturday(new Date(d));
        if (!isHoliday) {
            classesToSkip += timetable.filter(c => c.day === dayOfWeek.toString()).length;
            daysChecked++;
        }
    }

    const futureAttended = futureClasses - classesToSkip;
    const overallAttended = numAttended + futureAttended;
    const overallConducted = numConducted + futureClasses;
  ...
  }, [userData, predictionDays]);
  
  const { currentPercent, totalClasses, attendedClasses, conductedClasses, futurePercent } = calculateAttendance();
  ...
  return (
    <div className="p-4 md:p-8 text-gray-800 dark:text-gray-200 h-full overflow-y-auto">
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg">
          <h2 className="text-2xl font-bold mb-4">Attendance Prediction</h2>
          <div className="flex items-center space-x-4 mb-4">
              <span>If I skip the next</span>
              <input 
                  type="number" 
                  value={predictionDays}
                  onChange={(e) => setPredictionDays(Math.max(1, parseInt(e.target.value) || 1))}
                  className="w-20 p-2 text-center bg-gray-100 dark:bg-gray-700 rounded-md"
              />
              <span>working day(s) of classes...</span>
          </div>
          <p className="text-lg">Your predicted attendance will be: <span className="font-bold text-2xl text-orange-500">{futurePercent}%</span></p>
      </div>
    </div>
  );
}
...
function Settings({ user, userData }) {
  ...
  const [conductedClasses, setConductedClasses] = useState(0); // NEW
  const [attendedClasses, setAttendedClasses] = useState(0);   // NEW
    ...
    if (file) {
        ...
        reader.onload = (e) => {
            try {
                ...
                alert("Failed to parse timetable file. Please ensure it's a valid JSON array of classes.");
            }
        };
  ...
  return (
    <div className="p-4 md:p-8 text-gray-800 dark:text-gray-200 h-full overflow-y-auto">
      ...
      <div className="space-y-8 max-w-4xl mx-auto">
        ...
        {/* NEW: Simplified Attendance Input */}
        ...
        <SettingsCard title="Class Timetable">
            <div className="mb-4">
                <label htmlFor="timetable-upload" className="w-full inline-flex items-center justify-center px-4 py-2 border border-dashed border-gray-400 dark:border-gray-600 rounded-md cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    <UploadIcon />
                    <span className="ml-2">Upload Timetable (JSON)</span>
                </label>
                <input id="timetable-upload" type="file" className="hidden" accept=".json" onChange={handleTimetableUpload} />
                <p className="text-xs text-gray-500 mt-1">
                    {'Upload a JSON file. Example format: [{"name":"Math","time":"10:00","day":"1"}] (day 1 is Monday)'}
                </p>
            </div>
        ...
        <SettingsCard title="Holidays">
            <div className="mb-6">
                ...
                <p className="text-sm text-gray-500 mb-4">Sundays, 2nd & 4th Saturdays, and major public holidays are automatically included. You only need to add custom holidays for your specific institution below.</p>
            </div>
